...
use zerotier_crypto::constant::{AES_256_KEY_SIZE, AES_GCM_NONCE_SIZE, AES_GCM_TAG_SIZE};
...
use zerotier_utils::indexed_heap::{BinaryHeapIndex, IndexedBinaryHeap};
...
pub struct Session<Application: ApplicationLayer> {
    ...
    remote_s_public_key: P384PublicKey,
    ...
    header_send_cipher: Aes<true>,
    header_receive_cipher: Aes<false>,
    kex_send_cipher: Mutex<Option<AesGcm<true>>>,
    kex_receive_cipher: Mutex<Option<AesGcm<false>>>,
...
struct SessionMutableState<Application: ApplicationLayer> {
    ...
    cipher_states: [Option<SessionKey>; 2],
...
enum OfferStateMachine<Application: ApplicationLayer> {
    ...
    NoiseKKPattern2 {
        ...
        timeout: i64,
        noise_message: [u8; NoiseKKPattern1or2::SIZE],
        kex_send_key: Secret<AES_256_KEY_SIZE>,
...
pub(crate) struct NoiseXKBobHandshakeState<Application: ApplicationLayer> {
    ...
    header_receive_key: Secret<AES_256_KEY_SIZE>,
    header_send_key: Secret<AES_256_KEY_SIZE>,
    ratchet_number: u64,
    ...
    noise_k_eseeekem1psk: Secret<AES_256_KEY_SIZE>,
...
struct NoiseXKAliceHandshake<Application: ApplicationLayer> {
    ...
    offer: NoiseXKAliceHandshakeState,
}
enum NoiseXKAliceHandshakeState {
    NoiseXKPattern1 {
...
struct SessionKey {
    ...
    receive_cipher_pool: [Mutex<AesGcm<false>>; 8],
    /// Pool of reusable receiving ciphers.
    send_cipher_pool: [Mutex<AesGcm<true>>; 8],
...
impl<Application: ApplicationLayer> Context<Application> {
    ...
    pub fn new() -> Self {
        ...
        Self(Arc::new(ContextInner {
            ...
            challenge_salt: random::get_bytes_secure(),
        }))
    }
    ...
    pub fn open(
        ...
        let sha512 = &mut SHA512::new();

        let alice_s_keypair = app.local_s_keypair();
        let noise_kk_ss = alice_s_keypair.agree(&remote_s_public_key).ok_or(OpenError::InvalidPublicKey)?;
        let noise_kk_local_init_h = mix_hash(sha512, &INITIAL_H_REKEY, alice_s_keypair.public_key_bytes());
        ...
        let noise_kk_remote_init_h = mix_hash(sha512, &noise_kk_remote_init_h, alice_s_keypair.public_key_bytes());

        let mut session_queue = self.0.session_queue.lock().unwrap();
        let mut session_map = self.0.session_map.write().unwrap();
        let local_key_id = generate_key_id(&session_map);
        // Begin Noise XKhfs+psk2.
        let (offer, a2b_header_key, b2a_header_key) =
            NoiseXKAliceHandshake::<Application>::initialize(local_key_id, &remote_s_public_key, &ratchet_fingerprint)?;
        ...
        let session = Arc::new(Session {
            ...
            header_receive_cipher: Aes::new(b2a_header_key.as_bytes()),
            header_send_cipher: Aes::new(a2b_header_key.as_bytes()),
    ...
    pub fn receive<'a, SendFn: FnMut(&mut [u8]) -> bool>(
        ...
        check_accept_session: impl FnOnce(&P384PublicKey, &[u8], Option<&[u8; RATCHET_FINGERPRINT_SIZE]>) -> AcceptSessionAction<Application>,
        ...
        let (session, packet_type, fragments) = {
            ...
            if let Some(local_key_id) = NonZeroU32::new(u32::from_ne_bytes(local_key_id)) {
                ...
                if let Some((Some(session), key_index)) = session_map.get(&local_key_id).map(|r| (r.0.upgrade(), r.1 as usize)) {
                    drop(session_map);
                    session
                        .header_receive_cipher
                        .crypt_block_in_place(&mut incoming_physical_packet[HEADER_PROTECT_ENC_START..HEADER_PROTECT_ENC_END]);
                    ...
                    if packet_type == PACKET_TYPE_DATA {
                        ...
                        c.reset_init_gcm(&create_message_nonce(packet_type, incoming_counter));
                    ...
                    if let Some(incoming) = incoming.as_ref() {
                        Aes::<false>::new(incoming.header_receive_key.as_bytes())
                            .crypt_block_in_place(&mut incoming_physical_packet[HEADER_PROTECT_ENC_START..HEADER_PROTECT_ENC_END]);
        ...
        match packet_type {
            PACKET_TYPE_NOISE_XK_PATTERN_1 => {
                ...
                if let Some(remote_key_id) = NonZeroU32::new(u32::from_ne_bytes(noise_pattern1.alice_key_id)) {
                    ...
                    match check_allow_incoming_session() {
                        ...
                        IncomingSessionAction::Challenge => {
                            ...
                            let is_valid = self.check_challenge_window(counter)
                                && secure_eq(&hasher.0.finish()[..CHALLENGE_MAC_SIZE], &noise_pattern1.challenge_mac)
                            ...
                            if !is_valid {
                                ...
                                remote_address.hash(&mut hasher);
                                hasher.0.update(&self.0.challenge_salt);
                                challenge.challenge_mac.copy_from_slice(&hasher.0.finish()[..CHALLENGE_MAC_SIZE]);
                    ...
                    let local_s_keypair = app.local_s_keypair();
                    ...
                    let noise_h = mix_hash(sha512, &noise_h, local_s_keypair.public_key_bytes());
                    // Noise process pattern1 e token.
                    let mut noise_ck = SymmetricState::new(INITIAL_H);
                    let (noise_e_pattern1, noise_es) = from_bytes_agreement(&noise_pattern1.noise_e, &local_s_keypair)
                    ...
                    let noise_k_es = noise_ck.mix_key_initialize_key(noise_es.as_bytes());
                    drop(noise_es);
                    // Noise process pattern1 e1 token.
                    let (is_auth, noise_h_ee1) = decrypt_and_hash(
                        sha512,
                        &noise_k_es,
                        &noise_h_e,
                        packet_type,
                    ...
                    let (is_auth, noise_h_ee1p) = decrypt_and_hash(
                        sha512,
                        &noise_k_es,
                        &noise_h_ee1,
                        packet_type,
                    ...
                    let (header_b2a_key, header_a2b_key) = noise_ck.get_ask2(LABEL_HEADER_KEY, &noise_h_ee1p);
                    // Get ratchet key.
                    ...
                    let noise_e_pattern2_secret = P384KeyPair::generate();
                    ...
                    let noise_ee = noise_e_pattern2_secret
                        .agree(&noise_e_pattern1)
                        .ok_or(byzantine_fault!(FaultType::FailedAuthentication, false))?;
                    let noise_k_esee = noise_ck.mix_key_initialize_key(noise_ee.as_bytes());
                    drop(noise_ee);
                    // Noise process pattern2 ekem1 token.
                    let (noise_ekem1, noise_ekem1_secret) = pqc_kyber::encapsulate(&noise_pattern1.noise_e1, &mut random::SecureRandom::default())
                    ...
                    let noise_h_ee1peekem1 = encrypt_and_hash(
                        sha512,
                        &noise_k_esee,
                        &noise_h_ee1pe,
                    ...
                    drop(noise_k_esee);
                    noise_ck.mix_key(noise_ekem1_secret.as_bytes());
                    drop(noise_ekem1_secret);
                    // Noise process pattern2 psk token.
                    let (temp_h, noise_k_eseeekem1psk) = noise_ck.mix_key_and_hash_initialize_key(&ratchet_key);
                    ...
                    let local_key_id = generate_key_id(&self.0.session_map.read().unwrap());
                    let noise_pattern2: &mut NoiseXKPattern2 = byte_array_as_proto_buffer_mut(&mut message2);
                    noise_pattern2.bob_key_id = local_key_id.get().to_ne_bytes();

                    let noise_h_ee1peekem1pskp = encrypt_and_hash(
                        sha512,
                    ...
                    send_with_fragmentation(
                        ...
                        Some(&Aes::new(header_b2a_key.first_n::<AES_256_KEY_SIZE>())),
                    );
                    ...
                    if let Some(session) = self.0.session_map.read().unwrap().get(&local_key_id).and_then(|s| s.0.upgrade()) {
                        ...
                        if let NoiseXKPattern1or3(handshake_state) = &mut state.outgoing_offer {
                            if let NoiseXKAliceHandshakeState::NoiseXKPattern1 { noise_message, .. } = &mut handshake_state.offer {
                                ...
                                let mut pow = random::next_u64_secure();
                                let sha512 = &mut SHA512::new();
                                loop {
                                    let pattern1: &mut NoiseXKPattern1 = byte_array_as_proto_buffer_mut(noise_message);
            ...
            PACKET_TYPE_NOISE_XK_PATTERN_2 => {
                ...
                if let NoiseXKPattern1or3(handshake_state) = &state.outgoing_offer {
                    ...
                    } = &handshake_state.offer
                    {
                        ...
                        // Noise process pattern2 e token.
                        if let Some((noise_e_pattern2, noise_ee)) = from_bytes_agreement(&noise_pattern2.noise_e, &noise_e_secret) {
                            let sha512 = &mut SHA512::new();
                            ...
                            drop(noise_ee);
                            // Noise process pattern2 ekem1 token.
                            let (is_auth, noise_h_ee1peekem1) = decrypt_and_hash(
                                sha512,
                                &noise_k_esee,
                                &noise_h_ee1pe,
                                packet_type,
                            ...
                            let noise_pattern2: &NoiseXKPattern2 = byte_array_as_proto_buffer(message);
                            let noise_ekem1_secret =
                                pqc_kyber::decapsulate(&noise_pattern2.noise_ekem1, noise_e1_secret.as_bytes()).map(|k| Secret(k));
                            if let Some(Ok(noise_ekem1_secret)) = is_auth.then_some(noise_ekem1_secret) {
                                ...
                                let mut ratchet_key = state.ratchet_key.as_bytes();
                                ...
                                for i in 0..2 {
                                    ...
                                    // Noise process pattern2 psk token.
                                    let (temp_h, noise_k_ratchet) = noise_ck_ratchet.mix_key_and_hash_initialize_key(ratchet_key);
                                    let noise_h_ee1peekem1psk = mix_hash(sha512, &noise_h_ee1peekem1, &temp_h);
                                    // Noise process pattern2 payload.
                                    let (is_auth, noise_h_ratchet) =
                                        decrypt_and_hash(sha512, &noise_k_ratchet, &noise_h_ee1peekem1psk, packet_type, 0, &mut payload);
                                ...
                                if let Some((remote_key_id, noise_k_eseeekem1psk, noise_h_ee1peekem1pskp)) = ratchet_result {
                                    ...
                                    let alice_s_keypair = app.local_s_keypair();
                                    if let Some(noise_se) = alice_s_keypair.agree(&noise_e_pattern2) {
                                        ...
                                        message3[s_enc_start..s_auth_start].copy_from_slice(alice_s_keypair.public_key_bytes());
                                        let noise_h_ee1peekem1pskps = encrypt_and_hash(
                                            sha512,
                                            &noise_k_eseeekem1psk,
                                            &noise_h_ee1peekem1pskp,
                                            PACKET_TYPE_NOISE_XK_PATTERN_3,
                                        ...
                                        let noise_k_eseeekem1pskse = noise_ck.mix_key_initialize_key(noise_se.as_bytes());
                                        drop(noise_se);
                                        // Noise process pattern3 payload token.
                                        message3[p_enc_start..p_auth_start].copy_from_slice(payload);
                                        let noise_h_ee1peekem1pskpsp = encrypt_and_hash(
                                            sha512,
                                        ...
                                        let (new_ratchet_key, new_ratchet_fingerprint) =
                                            noise_ck.get_ask2(LABEL_RATCHET_STATE, &noise_h_ee1peekem1pskpsp);
                                        let result = app.save_ratchet_state(
                                            &session.remote_s_public_key,
                                            &session.application_data,
                                            SaveRatchetAction::SaveAsUnconfirmed,
                                            new_ratchet_number,
                                            new_ratchet_fingerprint.as_bytes(),
                                            new_ratchet_key.as_bytes(),
                                            current_time,
                                        );
                                        if result.is_err() {
                                            return Err(ReceiveError::RatchetIoError);
                                        }
                                        let (kex_key_b2a, kex_key_a2b) = noise_ck.get_ask2(LABEL_KEX_KEY, &noise_h_ee1peekem1pskpsp);

                                        let local_key_id = handshake_state.local_key_id;
                                        drop(state);
                                        let mut state = session.state.write().unwrap();
                                        session.kex_receive_cipher.lock().unwrap().replace(AesGcm::new(kex_key_b2a.as_bytes()));
                                        session.kex_send_cipher.lock().unwrap().replace(AesGcm::new(kex_key_a2b.as_bytes()));

                                        state.cipher_states[0].replace(SessionKey::new::<Application>(
                                            noise_ck,
                                            local_key_id,
                                            remote_key_id,
                                            INIT_COUNTER,
                        ...
                        let ratchet_fingerprint = state.ratchet_fingerprint.clone();
                        if let NoiseXKPattern1or3(handshake_state) = &mut state.outgoing_offer {
                            if !handshake_state.reinitialize(&session, &ratchet_fingerprint, &mut self.0.session_map.write().unwrap(), current_time) {
                                session.expire()
                            }
                        }
                        drop(state);
            ...
            PACKET_TYPE_NOISE_XK_PATTERN_3 => {
                ...
                // Noise process pattern3 s token.
                let sha512 = &mut SHA512::new();
                let (is_auth, noise_h_ee1peekem1pskps) = decrypt_and_hash(
                    sha512,
                ...
                // Noise process pattern3 se token.
                if let Some((remote_s_public_key, noise_se)) =
                    from_bytes_agreement(&message[s_enc_start..s_auth_start], &handshake_state.noise_e_secret)
                {
                    let mut noise_ck = handshake_state.noise_ck_eseeekem1psk.clone();
                    let noise_k_eseeekem1pskse = noise_ck.mix_key_initialize_key(noise_se.as_bytes());
                    drop(noise_se);
                    // Noise process pattern3 payload.
                    let (is_auth, noise_h_ee1peekem1pskpsp) = decrypt_and_hash(
                        sha512,
                        &noise_k_eseeekem1pskse,
                        &noise_h_ee1peekem1pskps,
                        packet_type,
                    ...
                    let header_send_cipher = Aes::new(handshake_state.header_send_key.as_bytes());
                    let (kex_key_b2a, kex_key_a2b) = noise_ck.get_ask2(LABEL_KEX_KEY, &noise_h_ee1peekem1pskpsp);
                    match check_accept_session(
                        ...
                        AcceptSessionAction::Accept(application_data) => {
                            let bob_s_keypair = app.local_s_keypair();
                            let noise_kk_ss = bob_s_keypair
                                .agree(&remote_s_public_key)
                                .ok_or(byzantine_fault!(FaultType::FailedAuthentication, false))?;
                            let noise_kk_local_init_h = mix_hash(sha512, &INITIAL_H_REKEY, bob_s_keypair.public_key_bytes());
                            let noise_kk_local_init_h = mix_hash(sha512, &noise_kk_local_init_h, remote_s_public_key.as_bytes());
                            let noise_kk_remote_init_h = mix_hash(sha512, &INITIAL_H_REKEY, remote_s_public_key.as_bytes());
                            let noise_kk_remote_init_h = mix_hash(sha512, &noise_kk_remote_init_h, bob_s_keypair.public_key_bytes());

                            let (new_ratchet_key, new_ratchet_fingerprint) = noise_ck.get_ask2(LABEL_RATCHET_STATE, &noise_h_ee1peekem1pskpsp);
                            ...
                            let session = Arc::new(Session {
                                ...
                                header_receive_cipher: Aes::new(handshake_state.header_receive_key.as_bytes()),
                                header_send_cipher,
                                kex_receive_cipher: Mutex::new(Some(AesGcm::new(kex_key_a2b.as_bytes()))),
                                kex_send_cipher: Mutex::new(Some(AesGcm::new(kex_key_b2a.as_bytes()))),
                                noise_kk_ss,
                                noise_kk_local_init_h,
                                noise_kk_remote_init_h,
                                defrag: std::array::from_fn(|_| Mutex::new(Fragged::new())),
                        ...
                        AcceptSessionAction::SendReject => {
                            ...
                            let (mut fragment, len) = encrypt_control(
                                &mut AesGcm::new(kex_key_b2a.as_bytes()),
                                &header_send_cipher,
    ...
    pub fn send(
        ...
        for fragment_no in 0..fragment_count {
            ...
            session
                .header_send_cipher
                .crypt_block_in_place(&mut mtu_sized_buffer[HEADER_PROTECT_ENC_START..HEADER_PROTECT_ENC_END]);
...
fn initiate_rekey<Application: ApplicationLayer>(
    ...
    let noise_temp_h = noise_ck.mix_key_and_hash(state.ratchet_key.as_bytes());
    let noise_h_psk = mix_hash(sha512, &session.noise_kk_local_init_h, &noise_temp_h);
    // Noise process pattern1 e token.
    let noise_e_secret = P384KeyPair::generate();
    ...
    // Noise process pattern1 es token.
    let noise_es = noise_e_secret.agree(&session.remote_s_public_key).ok_or(())?;
    noise_ck.mix_key(noise_es.as_bytes());
    drop(noise_es);
    // Noise process pattern1 ss token.
    let noise_k_pskesss = noise_ck.mix_key_initialize_key(session.noise_kk_ss.as_bytes());
    // Noise process pattern1 payload token.
    let mut session_map = context.session_map.write().unwrap();
    let new_key_id = generate_key_id(&session_map);
    ...
    let noise_h_pskep = encrypt_and_hash(
        sha512,
        &noise_k_pskesss,
        &noise_h_pske,
        PACKET_TYPE_NOISE_KK_PATTERN_1,
...
fn receive_control_fragment<'a, Application: ApplicationLayer, SendFn: FnMut(&mut [u8]) -> bool>(
    ...
    return match packet_type {
            ...
            if let Some(ratchet) = key_confirmed {
                ...
                if result.is_ok() {
                    if let NoiseKKPattern2 { kex_send_key, .. } = &state.outgoing_offer {
                        session.kex_send_cipher.lock().unwrap().replace(AesGcm::new(kex_send_key.as_bytes()));
                    }
                    state.ratchet_number = ratchet.0;
                    state.ratchet_fingerprint.overwrite(&ratchet.1);
                    state.ratchet_key.overwrite(&ratchet.2);
                    state.current_key ^= 1;
                    state.outgoing_offer = new_normal_state(current_time);
                } else {
                    return Err(ReceiveError::RatchetIoError);
                }
            }
            ...
            match &state.outgoing_offer {
                KeyConfirm { .. } => {
                    let result = app.save_ratchet_state(
                        ...
                        state.ratchet_fingerprint.as_bytes(),
                        state.ratchet_key.as_bytes(),
                        current_time,
                    );
                    if result.is_ok() {
                        state.outgoing_offer = new_normal_state(current_time);
        ...
        PACKET_TYPE_NOISE_KK_PATTERN_1 => {
            ...
            // Noise process pattern1 e token.
            if let Some((alice_e, noise_es)) = from_bytes_agreement(&noise_pattern1.noise_e, &app.local_s_keypair()) {
                let bob_e_secret = P384KeyPair::generate();
                if let (Some(noise_ee), Some(noise_se)) = (bob_e_secret.agree(&alice_e), bob_e_secret.agree(&session.remote_s_public_key)) {
                    ...
                    // Noise process pattern1 payload.
                    let (is_auth, noise_h_pskep) = decrypt_and_hash(
                        sha512,
                        &noise_k_pskesss,
                        &noise_h_pske,
                        packet_type,
                    ...
                    if let (true, Some(remote_key_id)) = (is_auth, NonZeroU32::new(u32::from_ne_bytes(noise_pattern1.key_id))) {
                        ...
                        noise_ck.mix_key(bob_e_secret.public_key_bytes());
                        // Noise process pattern2 ee token.
                        noise_ck.mix_key(noise_ee.as_bytes());
                        drop(noise_ee);
                        // Noise process pattern2 se token.
                        let noise_k_pskessseese = noise_ck.mix_key_initialize_key(noise_se.as_bytes());
                        drop(noise_se);
                        // Noise process pattern2 payload.
                        ...
                        let new_key_id = chosen_id.unwrap_or_else(|| generate_key_id(&session_map));
                        ...
                        let (new_ratchet_key, new_ratchet_fingerprint) = noise_ck.get_ask2(LABEL_RATCHET_STATE, &noise_h_pskepep);
                        let result = app.save_ratchet_state(
                            &session.remote_s_public_key,
                            &session.application_data,
                            SaveRatchetAction::SaveAsUnconfirmed,
                        ...
                        let (kex_key_b2a, kex_key_a2b) = noise_ck.get_ask2(LABEL_KEX_KEY, &noise_h_pskepep);
                        ...
                        session.kex_receive_cipher.lock().unwrap().replace(AesGcm::new(kex_key_a2b.as_bytes()));

                        state.cipher_states[next_key_index].replace(SessionKey::new::<Application>(
                            noise_ck,
                            new_key_id,
                            remote_key_id,
                            current_counter,
        ...
        PACKET_TYPE_NOISE_KK_PATTERN_2 => {
            ...
            if let NoiseKKPattern1 { new_key_id, noise_e_secret, noise_ck, noise_h_pskep, .. } = &state.outgoing_offer {
                // Noise process pattern2 e token.
                if let Some((bob_e, noise_ee)) = from_bytes_agreement(&noise_pattern2.noise_e, &noise_e_secret) {
                    if let Some(noise_se) = app.local_s_keypair().agree(&bob_e) {
                        let sha512 = &mut SHA512::new();
                        let mut noise_ck = noise_ck.clone();
                        let noise_h_pskepe = mix_hash(sha512, noise_h_pskep, bob_e.as_bytes());
                        noise_ck.mix_key(bob_e.as_bytes());
                        // Noise process pattern2 ee token.
                        noise_ck.mix_key(noise_ee.as_bytes());
                        drop(noise_ee);
                        // Noise process pattern2 se token.
                        ...
                        drop(noise_se);
                        // Noise process pattern2 payload.
                        let (is_auth, noise_h_pskepep) = decrypt_and_hash(
                            sha512,
                            &noise_k_pskessseese,
                            &noise_h_pskepe,
                            packet_type,
...
impl<Application: ApplicationLayer> Session<Application> {
    ...
    pub fn ratchet_state(&self) -> (u64, [u8; RATCHET_FINGERPRINT_SIZE], [u8; RATCHET_KEY_SIZE]) {
        let state = self.state.read().unwrap();
        (state.ratchet_number, *state.ratchet_fingerprint.as_bytes(), *state.ratchet_key.as_bytes())
    }
...
impl<Application: ApplicationLayer> NoiseXKAliceHandshake<Application> {
    ...
    fn initialize(
    ...
    ) -> Result<(NoiseXKAliceHandshakeState, Secret<AES_256_KEY_SIZE>, Secret<AES_256_KEY_SIZE>), OpenError> {
        ...
        let noise_e_secret = P384KeyPair::generate();
        let noise_e1_secret = pqc_kyber::keypair(&mut random::SecureRandom::default());
        ...
        noise_ck.mix_key(noise_e_secret.public_key_bytes());
        // Noise process pattern1 es token.
        let noise_es = noise_e_secret.agree(remote_s_public_key).ok_or(OpenError::InvalidPublicKey)?;
        let noise_k_es = noise_ck.mix_key_initialize_key(noise_es.as_bytes());
        drop(noise_es);
        // Noise process pattern1 e1 token.
        ...
        // Noise process pattern1 payload.
        let noise_h_ee1p = encrypt_and_hash(
            sha512,
            &noise_k_es,
            &noise_h_ee1,
            PACKET_TYPE_NOISE_XK_PATTERN_1,
    ...
    fn reinitialize(
        ...
        if let Ok((offer, a2b_header_key, b2a_header_key)) =
            Self::initialize(local_key_id, &session.remote_s_public_key, ratchet_fingerprint.as_bytes())
        {
            ...
            self.offer = offer;
            session.header_send_cipher.reset(a2b_header_key.as_bytes());
            session.header_receive_cipher.reset(b2a_header_key.as_bytes());
            true
        } else {
            false
        }
...
fn new_normal_state<Application: ApplicationLayer>(current_time: i64) -> OfferStateMachine<Application> {
    OfferStateMachine::Normal {
        timeout: current_time
            .saturating_add(Application::REKEY_AFTER_TIME_MS)
            .saturating_sub(random::next_u32_secure() as i64 % Application::REKEY_AFTER_TIME_MAX_JITTER_MS),
    }
}
...
fn encrypt_and_hash(
    sha512: &mut SHA512,
    noise_k: &Secret<AES_256_KEY_SIZE>,
    ...
    let mut gcm = AesGcm::new(noise_k.as_bytes());
    // Encrypt and add authentication tag.
    gcm.reset_init_gcm(&create_message_nonce(packet_type, INIT_COUNTER + noise_k_uses));
    gcm.aad(noise_h);
    if auth_start > 0 {
        gcm.crypt_in_place(&mut message[..auth_start]);
    }
    message[auth_start..].copy_from_slice(&gcm.finish_encrypt());
...
fn decrypt_and_hash(
    ...
    (gcm.finish_decrypt(&message[auth_start..]), noise_h_c)
}
/// Encrypt a standardized control packet.
#[inline]
fn encrypt_control(
    c: &mut AesGcm<true>,
    header_cipher: &Aes<true>,
    packet_type: u8,
    counter: u64,
    remote_key_id: u32,
    packet: &[u8],
    ...
    c.reset_init_gcm(&create_message_nonce(packet_type, counter));
    if packet.len() > 0 {
        fragment[HEADER_SIZE..fragment_len - AES_GCM_TAG_SIZE].copy_from_slice(packet);
        c.crypt_in_place(&mut fragment[HEADER_SIZE..fragment_len - AES_GCM_TAG_SIZE]);
    }
    fragment[fragment_len - AES_GCM_TAG_SIZE..fragment_len].copy_from_slice(&c.finish_encrypt());
    drop(c);
    set_packet_header(&mut fragment, 1, 0, packet_type, remote_key_id, counter);
    header_cipher.crypt_block_in_place(&mut fragment[HEADER_PROTECT_ENC_START..HEADER_PROTECT_ENC_END]);
    (fragment, fragment_len)
}
#[inline]
fn decrypt_control<'a>(c: &mut AesGcm<false>, packet_type: u8, counter: u64, fragment: &'a mut [u8]) -> Result<&'a mut [u8], ReceiveError> {
    ...
    c.reset_init_gcm(&create_message_nonce(packet_type, counter));
    c.crypt_in_place(&mut fragment[HEADER_SIZE..fragment_len - AES_GCM_TAG_SIZE]);
    if !c.finish_decrypt(&fragment[fragment_len - AES_GCM_TAG_SIZE..fragment_len]) {
...
fn send_with_fragmentation(
    ...
    loop {
        ...
        if let Some(hcc) = header_cipher {
            hcc.crypt_block_in_place(&mut fragment[HEADER_PROTECT_ENC_START..HEADER_PROTECT_ENC_END]);
        }
        if !send(fragment) {
            return false;
        }
...
fn generate_key_id<Application: ApplicationLayer>(session_map: &HashMap<NonZeroU32, (Weak<Session<Application>>, bool)>) -> NonZeroU32 {
    loop {
        if let Some(local_key_id) = NonZeroU32::new(random::next_u32_secure()) {
...
impl SessionKey {
    ...
    fn new<Application: ApplicationLayer>(
        ...
        let (b2a, a2b) = ck.split();
        let (receive_key, send_key) = if is_bob {
            (&a2b, &b2a)
        } else {
            (&b2a, &a2b)
        };
        let receive_cipher_pool = std::array::from_fn(|_| Mutex::new(AesGcm::new(receive_key.as_bytes())));
        let send_cipher_pool = std::array::from_fn(|_| Mutex::new(AesGcm::new(send_key.as_bytes())));
        Self {
            local_key_id,
            remote_key_id,
            receive_cipher_pool,
    ...
    #[inline(always)]
    fn get_send_cipher<'a>(&'a self, counter: u64) -> Result<MutexGuard<'a, AesGcm<true>>, SendError> {
...
#[inline]
fn mix_hash(hasher: &mut SHA512, h: &[u8; NOISE_HASHLEN], m: &[u8]) -> [u8; NOISE_HASHLEN] {
    hasher.reset();
    hasher.update(h);
    hasher.update(m);
    hasher.finish()
...
/// zeros.
#[inline]
fn verify_pow<Application: ApplicationLayer>(hasher: &mut SHA512, message: &[u8]) -> bool {
    if Application::PROOF_OF_WORK_BIT_DIFFICULTY == 0 {
        return true;
    }
    hasher.reset();
    hasher.update(&message[NoiseXKPattern1::P_AUTH_END..NoiseXKPattern1::SIZE]);
    let mut n = 0u32.to_ne_bytes();
    n.copy_from_slice(&hasher.finish()[..4]);
    let n = u32::from_be_bytes(n);
    n.leading_zeros() >= Application::PROOF_OF_WORK_BIT_DIFFICULTY
}
fn from_bytes_agreement(public: &[u8], private: &P384KeyPair) -> Option<(P384PublicKey, Secret<48>)> {
    P384PublicKey::from_bytes(public).and_then(|e| private.agree(&e).map(|ee| (e, ee)))
}
